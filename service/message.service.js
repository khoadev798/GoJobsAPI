...
let newMessage = async (message) => {
  ...
  if (message.content == undefined) {
    ...
    if (message.empId == message.content[0].userId) {
      ...
      jobService.FCM.sendToMultipleToken(
        message,
        flcTokenDevices._doc.flcId.flcTokenDevice,
        (err, response) => {
          if (err) {
            console.log("err--", err);
          } else {
      ...
      jobService.FCM.sendToMultipleToken(
        message,
        empTokenDevices._doc.empId.empTokenDevice,
        (err, response) => {
          if (err) {
            console.log("err--", err);
          } else {
  ...
  return {
    ...
    messages: findMessageResult.chat,
  };
};
...
let findMessageByUserId = async (message) => {
  let found = await MessageModel.findOne(
    {
      $and: [{ empId: message.empId }, { flcId: message.flcId }],
    },
    {},
    (err, doc) => {
...
let findMessageByUserIdOr = async (message) => {
  let found = await MessageModel.findOne(
    {
      $or: [{ empId: message.empId }, { flcId: message.flcId }],
    },
    {},
    (err, doc) => {
      if (err) handlerError(err);
