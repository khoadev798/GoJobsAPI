...
let getNotificationMessageByEmp = async (req, res) => {
  ...
  res
    .status(getNotificationMessageResult.code)
    .send(getNotificationMessageResult.listNotification);
};
...
let getMessageDetail = async (req, res) => {
  ...
  let getMessageDetailResult = await messageService.getMessageDetail({
    _id,
    pageNumber,
    pageSize,
  });
  res
    .status(getMessageDetailResult.code)
    .send(getMessageDetailResult.messageDetail);
};
module.exports = {
  newMessage,
